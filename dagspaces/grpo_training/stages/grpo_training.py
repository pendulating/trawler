...
from .vllm_weight_sync_patches import apply_colocate_patches
...
def run_grpo_training_stage(
    ...
    # ── Colocate weight-sync runtime patches (per model family) ──
    # TRL gives no passthrough for its colocate engine kwargs and no hook on its
    # weight-sync path, so three corrections install as method wrappers. They
    # live in stages/vllm_weight_sync_patches.py; the install ORDER matters and
    # that module owns it. See its docstring for the architecture notes.
    ...
    _applied_patches = apply_colocate_patches(
        model=model,
        model_family=model_family,
        model_source=_original_model_source,
        use_vllm=use_vllm,
        vllm_mode=vllm_mode,
        grpo_cfg=grpo_cfg,
    )
    print(f"[grpo_training] colocate patches applied: {_applied_patches or 'none'}")
