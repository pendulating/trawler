...
def _run_transformers_vlm_inference(
    ...
    sys_msg: str = "",
    usr_text: str = "",
    sys_msg_col: str | None = None,
    usr_text_col: str | None = None,
) -> pd.DataFrame:
    ...
    Args:
        ...
        sys_msg_col: Optional per-row system message column; overrides
            sys_msg row-wise (hypothetical variants).
        usr_text_col: Optional per-row user text column; overrides usr_text
            row-wise.
    """
    import torch
    ...
    def _build_mllama_prompt(sys_text: str, user_text: str) -> str | None:
        if not (sys_text or user_text):
            return None
    ...
    mllama_prompt = _build_mllama_prompt(sys_msg, usr_text)
    per_row_prompts = bool(sys_msg_col and usr_text_col)
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        ...
        if per_row_prompts:
            text = _build_mllama_prompt(str(row[sys_msg_col]), str(row[usr_text_col])) or prompt_text
        else:
...
def run_vlm_inference(
    ...
    Args:
        ...
        prompt_col: Optional column of pre-rendered per-row prompts (e.g.
            hypothetical variants); overrides prompt_text row-wise.
        sys_msg_col: Per-row system message column (transformers fallback
            counterpart of prompt_col).
        usr_text_col: Per-row user text column (transformers fallback
            counterpart of prompt_col).
    ...
    if prompt_col and prompt_col not in df.columns:
        raise ValueError(f"prompt_col {prompt_col!r} not in DataFrame columns")
    ...
    if model_family in _TRANSFORMERS_FALLBACK_FAMILIES:
        if prompt_col and not (sys_msg_col and usr_text_col):
            raise ValueError(
                "Per-row prompts on the transformers fallback path need "
                "sys_msg_col and usr_text_col (raw text; prompt_col holds a "
                "vLLM-rendered template the processor cannot consume)"
            )
        return _run_transformers_vlm_inference(
            ...
            sys_msg_col=sys_msg_col, usr_text_col=usr_text_col,
        )
    ...
    for idx, row in df.iterrows():
        img_path = str(row[image_col])
        row_prompt = str(row[prompt_col]) if prompt_col else prompt_text
        try:
