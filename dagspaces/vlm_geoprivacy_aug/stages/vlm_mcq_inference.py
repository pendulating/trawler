...
When the incoming dataset is hypothetical-expanded (has ``hyp_id``), prompts
vary per row: each variant's capture-context frame is rendered into its own
prompt and inference runs with per-row prompts. Otherwise this behaves
exactly like the un-augmented benchmark.
"""
...
def run_mcq_inference(df: pd.DataFrame, cfg: DictConfig) -> pd.DataFrame:
    ...
    if "hyp_id" in df.columns:
        df = attach_variant_prompts(df, cfg, is_free_form=False)
        prompt_kwargs = {
        ...
        print(f"[vlm_mcq_inference] Hypothetical mode: {n_variants} variants, {len(df)} rows")
