"""Orchestrator for the vlm_geoprivacy_aug dagspace.

Thin wrapper over the shared eval run loop in ``dagspaces/common/orchestrator.py``
(see ``wiki/jul19_orchestrator_unification_plan.md``).  The only vlm_geoprivacy_aug-specific
...
# ``importlib.import_module("dagspaces.vlm_geoprivacy_aug.orchestrator").get_stage_registry()``.
...
ORCHESTRATOR_HOOKS = OrchestratorHooks(
    dagspace_module="dagspaces.vlm_geoprivacy_aug.orchestrator",
    dagspace_name="vlm_geoprivacy_aug",
    output_subdir="vlm_geoprivacy_aug",
    job_prefix="VLM",
    config_dir=_CONF_DIR,
    log_eval_metrics=_log_eval_metrics,
    wandb_dagspace=lambda cfg: "vlm_geoprivacy_aug",
    use_srun=False,
)


def run_experiment(cfg: DictConfig) -> None:
    """Execute the vlm_geoprivacy_aug evaluation pipeline."""
