...
def prepare_question_prompt(
    ...
    Args:
        hypothetical: Optional capture-context variant to "inpaint" into the
            prompt (see ``hypotheticals.py``). ``None`` or the baseline
            variant leaves the prompt byte-identical to the un-augmented
            benchmark.
    ...
    if hypothetical is not None and not hypothetical.is_baseline:
        if hypothetical.position == "system_suffix":
            ...
            usr_prompts.append(render_user_frame(hypothetical))
