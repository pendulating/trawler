"""Risks and Benefits classification profile implementation - standalone module following decompose_nbl.py pattern."""
...
def get_risks_benefits_guided_json_schema() -> Dict[str, Any] | None:
    """Get JSON schema for Risks and Benefits guided decoding."""
    try:
        return {
            "type": "object",
            "properties": {
                ...
                "Assessment of impact on Human Rights": {
                    ...
                    "required": ["Positive Impacts", "Negative Impacts"],
...
def format_user_prompt(row: Dict[str, Any], user_template: str, missing_placeholder: str) -> str:
    """Format user prompt for Risks and Benefits classification."""
    ...
    try:
        ...
        try:
            if hasattr(sampling_params, "get"):
                max_output = int(sampling_params.get("max_tokens", 2048) or 2048)
            else:
                max_output = int((sampling_params or {}).get("max_tokens", 2048))
        except Exception:
            max_output = 2048
        safety = 512
...
def run_classification_risks_benefits(df: Any, cfg) -> Any:
    """
    Risks and Benefits classification implementation - standalone module.
    
    Follows the decompose_nbl.py pattern with complete implementation including:
    - Ray initialization
    - Engine config setup
    - EU vague flags filtering
    - Guided decoding with Risks/Benefits JSON schema (complex nested structure)
    ...
    # Support prompt subdirectory override (e.g., 'general_ai' for general_ai/classify_risks_and_benefits.yaml)
    prompt_subdir = getattr(cfg.runtime, "prompt_subdirectory", None)
    prompt_filename = "classify_risks_and_benefits.yaml"
    if prompt_subdir:
    ...
    use_llm = bool(getattr(cfg.runtime, "use_llm_classify", True))  # RB profile defaults to LLM
    ...
    if not use_llm or not _RAY_OK:
        ...
        if is_ray_ds:
            def _empty_batch(pdf: pd.DataFrame) -> pd.DataFrame:
                pdf = pdf.copy()
                pdf["rb_desc"] = None
                pdf["rb_human_rights"] = None
                pdf["rb_sdgs"] = None
                pdf["rb_additional"] = None
        ...
        out["rb_desc"] = None
        out["rb_human_rights"] = None
        out["rb_sdgs"] = None
        out["rb_additional"] = None
    ...
    try:
        ...
        print(f"[classify_risks_benefits] DEBUG: sp_src type={type(sp_src)}, value={sp_src}", flush=True)
        ...
        print(f"[classify_risks_benefits] DEBUG: After to_container, sampling_params={sampling_params}, max_tokens={sampling_params.get('max_tokens', 'NOT SET')}", flush=True)
    except Exception as e:
        print(f"[classify_risks_benefits] DEBUG: Exception getting sampling_params: {e}", flush=True)
    ...
    try:
        ...
        print(f"[classify_risks_benefits] Using max_tokens={actual_max_tokens} for LLM generation (from config)", flush=True)
    except Exception:
        pass
    
    # Risks/Benefits guided JSON schema
    RISKS_BENEFITS_GUIDED_JSON_SCHEMA = get_risks_benefits_guided_json_schema()
    ...
    def _normalize_na_blanks(r: Dict[str, Any]) -> Dict[str, Any]:
        # Normalize EU input fields (same as EU profile)
        eu_keys = [
            "deployment_domain",
            "deployment_purpose",
    ...
    def _pre(row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if tok_local:
                try:
                    ...
                    print(f"[classify_risks_benefits] Prompt length: system={system_tokens}, user={user_tokens}, total={total_prompt_tokens} tokens", flush=True)
                except Exception:
                    # Fallback to character count if tokenizer fails
                    system_chars = len(system_prompt)
                    user_chars = len(user)
                    total_chars = system_chars + user_chars
                    print(f"[classify_risks_benefits] Prompt length (chars): system={system_chars}, user={user_chars}, total={total_chars} chars", flush=True)
            else:
                system_chars = len(system_prompt)
                user_chars = len(user)
                total_chars = system_chars + user_chars
                print(f"[classify_risks_benefits] Prompt length: system={system_chars}, user={user_chars}, total={total_chars} chars", flush=True)
        except Exception as e:
            print(f"[classify_risks_benefits] Could not calculate prompt length: {e}", flush=True)
        
        # Sanitize and stabilize sampling params
        sp_local = sanitize_for_json(dict(sampling_params or {}))
        print(f"[classify_risks_benefits] DEBUG: After sanitize_for_json, sp_local max_tokens={sp_local.get('max_tokens', 'NOT SET')}", flush=True)
        ...
        try:
            ...
            if isinstance(row_max_tokens, (int, float)):
                print(f"[classify_risks_benefits] Row max_tokens={row_max_tokens}", flush=True)
        except Exception:
            pass
        ...
        # Add guided decoding
        if RISKS_BENEFITS_GUIDED_JSON_SCHEMA is not None:
            try:
                ...
                try:
                    print(f"[classify_risks_benefits] Failed to build guided_decoding dict: {exc}", flush=True)
                except Exception:
                    pass
                try:
        ...
        print(f"[classify_risks_benefits] DEBUG: Before _stabilize_sampling_params, sp_local max_tokens={sp_local.get('max_tokens', 'NOT SET')}", flush=True)
        sp_local = _stabilize_sampling_params(sp_local)
        print(f"[classify_risks_benefits] DEBUG: After _stabilize_sampling_params, sp_local max_tokens={sp_local.get('max_tokens', 'NOT SET')}", flush=True)
    ...
    def _post(row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(f"[classify_risks_benefits] _post called: usage={usage is not None}, raw_len={len(raw)}", flush=True)
        print(f"[classify_risks_benefits] _post row keys: {list(row.keys())[:20]}", flush=True)  # Log first 20 keys to see what's available
        try:
            if usage:
                ...
                print(
                    f"[classify_risks_benefits] Token usage: prompt={prompt_tokens}, output={output_tokens}, "
                ...
                if finish_reason == "length":
                    print(
                        f"[classify_risks_benefits] WARNING: Output truncated due to length limit! "
                        f"prompt_tokens={prompt_tokens} + max_tokens={max_tokens} may exceed max_model_len",
                        flush=True,
                    )
            else:
                # Check alternative locations for usage info
                print(f"[classify_risks_benefits] WARNING: usage is None or empty. Row keys: {list(row.keys())[:20]}", flush=True)
                # Try to find usage in other possible locations
                if "token_usage_prompt" in row or "token_usage_output" in row:
                    print(f"[classify_risks_benefits] Found token_usage columns: prompt={row.get('token_usage_prompt')}, output={row.get('token_usage_output')}", flush=True)
        except Exception as e:
            print(f"[classify_risks_benefits] ERROR in token usage logging: {e}", flush=True)
            import traceback
            traceback.print_exc()
        ...
        if isinstance(parsed, dict):
            ...
            try:
                ...
                rb_human_rights = to_json_str(hr) if hr is not None else None
            except Exception:
                rb_human_rights = None
            try:
                sdgs = parsed.get("Assessment of impact on Sustainable Development Goals")
                rb_sdgs = to_json_str(sdgs) if sdgs is not None else None
            except Exception:
                rb_sdgs = None
            try:
                additional = parsed.get("Assessment of additional impacts")
                rb_additional = to_json_str(additional) if additional is not None else None
            except Exception:
                rb_additional = None
        else:
            try:
                print(
                    "[classify_risks_benefits] Failed to parse LLM JSON output; full raw output:\n"
                    f"{raw}",
                    flush=True,
                )
            except Exception:
        ...
        try:
            rb_raw_json_str = to_json_str(parsed)
        except Exception:
            rb_raw_json_str = None
    ...
    if is_ray_ds:
        ...
        try:
            ...
            print(f"[classify_risks_benefits] Incoming Ray dataset columns ({len(cols)}): {cols}", flush=True)
            try:
                ...
                print(
                    "[classify_risks_benefits] Incoming schema field types: " + ", ".join(schema_desc),
                    flush=True,
                )
            except Exception:
                pass
        ...
        try:
            ...
            current_cols = set()
        allowed_columns = get_required_input_columns(False, True)  # risks_benefits profile
        ...
        try:
            if allowed_columns and allowed_columns != current_cols:
                ...
                print(
                    f"[classify_risks_benefits] Pruned dataset columns to {sorted(allowed_columns)}",
                    flush=True,
                )
        except Exception:
            pass
        ...
        try:
            ...
            print(f"[classify_risks_benefits] Columns after pruning ({len(new_cols)}): {new_cols}", flush=True)
            try:
                import pyarrow as pa  # type: ignore
                new_schema_desc = [
                    f"{field.name}:{field.type}" if isinstance(field, pa.Field) else str(field)
                ...
                print(
                    "[classify_risks_benefits] Post-prune schema field types: "
        ...
        try:
            ...
            try:
                ...
                print(f"[classify_risks_benefits] Filtered {vague_count} too-vague rows before LLM processing", flush=True)
            except Exception:
                pass
        except Exception:
            ds_vague = None
        ...
        try:
            ...
            print(f"[classify_risks_benefits] Warning: Failed to drop internal columns: {e}", flush=True)
        
        # Merge vague rows back
        skip_merge_filtered = bool(getattr(cfg.runtime, "skip_merge_filtered", False))
        print(
            f"[classify_risks_benefits] Checking merge conditions: ds_vague={ds_vague is not None}, skip_merge_filtered={skip_merge_filtered}",
            flush=True,
        )
        if skip_merge_filtered:
            ...
            print(f"[classify_risks_benefits] skip_merge_filtered enabled; returning {len(df_llm)} rows without merging vague", flush=True)
            return df_llm
        
        if ds_vague is not None:
            print(f"[classify_risks_benefits] Starting merge process...", flush=True)
            try:
                df_llm = ds_llm_results.to_pandas()
                df_vague_pd = ds_vague.to_pandas().copy()
                
                result_parts = [normalize_profile_columns(df_llm, False, True)]
                
                print(f"[classify_risks_benefits] Merging back {len(df_vague_pd)} too-vague rows to output", flush=True)
                df_vague_pd["classification_mode"] = "too_vague_to_process"
                df_vague_pd["too_vague_to_process"] = True
                df_vague_pd = normalize_profile_columns(df_vague_pd, False, True)
                ...
                result_df = prune_result_columns(result_df, False, True)
                try:
                    num_vague = len(df_vague_pd) if 'df_vague_pd' in locals() else 0
                except Exception:
                    num_vague = 0
                print(f"[classify_risks_benefits] Merged results: {len(df_llm)} LLM-processed + {num_vague} too-vague = {len(result_df)} total", flush=True)
                return result_df
            except Exception as e:
                print(f"[classify_risks_benefits] ERROR: Failed to merge vague rows: {e}", flush=True)
                import traceback
                traceback.print_exc()
                return ds_llm_results
        
        print(f"[classify_risks_benefits] No merge needed, returning LLM results directly", flush=True)
        return ds_llm_results
    ...
    try:
        schema = ds_in.schema()
        cols = list(schema.names)
        print(f"[classify_risks_benefits] (pandas path) incoming dataset columns ({len(cols)}): {cols}", flush=True)
        try:
            import pyarrow as pa  # type: ignore
            schema_desc = [
                f"{field.name}:{field.type}" if isinstance(field, pa.Field) else str(field)
            ...
            print(
                "[classify_risks_benefits] (pandas path) incoming schema field types: "
                + ", ".join(schema_desc),
                flush=True,
            )
        except Exception:
    ...
    try:
        ...
        current_cols = set()
    allowed_columns = get_required_input_columns(False, True)
    allowed_columns = allowed_columns & current_cols
    allowed_columns.update({col for col in ("article_id", "article_text") if col in current_cols})
    try:
        if allowed_columns and allowed_columns != current_cols:
            ds_in = ds_in.select_columns(sorted(allowed_columns))
            print(
                f"[classify_risks_benefits] (pandas path) pruned dataset columns to {sorted(allowed_columns)}",
                flush=True,
            )
            try:
                new_schema = ds_in.schema()
                new_cols = list(new_schema.names)
                print(
                    f"[classify_risks_benefits] (pandas path) columns after pruning ({len(new_cols)}): {new_cols}",
                    flush=True,
                )
                try:
                    import pyarrow as pa  # type: ignore
                    ...
                    print(
                        "[classify_risks_benefits] (pandas path) post-prune schema field types: "
                        + ", ".join(new_schema_desc),
                        flush=True,
                    )
                except Exception:
    ...
    # Merge vague rows back
    print(f"[classify_risks_benefits] Checking merge conditions: ds_vague={ds_vague is not None}", flush=True)
    if ds_vague is not None:
        print(f"[classify_risks_benefits] Starting merge process...", flush=True)
        try:
            df_llm = out_df
            ...
            try:
                ...
                num_vague = 0
            print(f"[classify_risks_benefits] Merged results: {len(df_llm)} LLM-processed + {num_vague} too-vague = {len(out_df)} total", flush=True)
        except Exception as e:
            print(f"[classify_risks_benefits] ERROR: Failed to merge vague rows: {e}", flush=True)
            import traceback
            traceback.print_exc()
    ...
    out_df = prune_result_columns(out_df, False, True)
    return out_df
