"""Orchestrator for CONFAIDE dagspace.
...
def _get_wandb_logger(cfg: DictConfig, stage: str, run_id: Optional[str] = None, run_config: Optional[Dict[str, Any]] = None):
    ...
    pipeline_id = pipeline_run_id(cfg, dagspace="confaide")
...
def execute_stage_job(context_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    run_config = build_run_config(cfg, node, context.inputs, context.output_paths, dagspace_name="confaide")
...
def run_experiment(cfg: DictConfig) -> None:
    ...
    with _get_wandb_logger(cfg, stage="orchestrator", run_id="monitor") as logger:
        try:
            ...
            if hydra_output_dir:
                output_root = os.path.join(hydra_output_dir, "confaide")
            else:
            ...
            for node_key in ordered_nodes:
                ...
                if node.launcher and _SUBMITIT_AVAILABLE:
                    ...
                    executor = _create_submitit_executor(launcher_cfg, f"CONFAIDE-{node.key}", log_folder, use_srun=False)
