...
# Public API
# ---------------------------------------------------------------------------
# Every name below is imported by at least one module outside this file, so it
# is API whether or not it is spelled like one. Twelve of them carried a
# leading underscore until 2026-08-12 while three dagspaces and the shared
# runner base imported them anyway — a private marker that nothing honoured
# tells a reader the wrong thing about what is safe to change.
#
# Anything NOT listed here is genuinely internal. Two such helpers
# (``_mirror_stage_metrics``, ``_rebuild_node``) are reached only by tests,
# which is a deliberate exception: a test may inspect internals.
#
# tests/common/test_orchestrator_api.py fails if production code imports a
# private name from this module again.
__all__ = [
    # Pipeline dataclasses and context
    "ArtifactRegistry",
    "OrchestratorHooks",
...
def merge_overrides(
    base_cfg: DictConfig, overrides: Mapping[str, Any] | None
) -> DictConfig:
    if not overrides:
        return base_cfg
...
def common_parent(paths: Iterable[str]) -> str | None:
    try:
...
def _resolve_pipeline_name() -> str | None:
...
def _resolve_eval_task(cfg: DictConfig, node: PipelineNodeSpec) -> str | None:
...
def _resolve_checkpoint_name(cfg: DictConfig) -> str | None:
...
def prepare_stage_input(
...
) -> tuple[pd.DataFrame | None, Any | None, bool]:
    ...
    columns = (
        dict(getattr(cfg.data, "columns", {})) if getattr(cfg, "data", None) else {}
    )
        ...
        try:
            ...
            if len(sample) > 0:
                ...
                if has_complex:
                    df[col] = df[col].apply(
                        lambda x: (
                            _json.dumps(x, default=_json_fallback)
...
def save_stage_outputs(out: pd.DataFrame, output_paths: dict[str, str]) -> None:
...
def collect_outputs(
    ...
    for key, path in context.output_paths.items():
        if os.path.exists(path):
            ...
            # Check fallback extensions written by save_stage_outputs
            found = False
...
def _adjust_tensor_parallel_env(valid_count: int, env_prefix: str = "UAIR") -> None:
    ...
    try:
        ...
        print(
            f"[orchestrator] WARNING: could not adjust {tp_env_name}: {exc}", flush=True
        )
...
def _log_gpu_environment(reason: str, env_prefix: str = "UAIR") -> None:
    ...
    try:
        ...
        dropped = [
            d.strip()
            for d in os.environ.get(f"{env_prefix}_SANITIZED_DROPPED_GPUS", "").split(
                ","
            )
            if d.strip()
        ]
        original = [
            d.strip()
            for d in os.environ.get(f"{env_prefix}_GPU_SANITIZE_ORIGINAL", "").split(
                ","
            )
            if d.strip()
        ]
...
def sanitize_cuda_visible_devices(
    ...
    if not valid:
        ...
        print_status({"gpu_sanitize_failures": failure_summary})
        raise RuntimeError(
...
def submit_slurm_job(
    ...
    try:
        ...
        raise RuntimeError(
            f"SLURM submission failed for '{node_key}' (launcher={launcher_name}): {e}"
        ) from e
    print_status({"node": node_key, "status": "submitted", "job_id": job.job_id})
    return job
...
def _resolve_hydra_output_dir() -> str | None:
...
def make_wandb_logger(
    ...
    *,
    ...
    wl = importlib.import_module(
        f"{hooks.dagspace_module.rsplit('.orchestrator', 1)[0]}.wandb_logger"
    )
    wb_config = wl.WandbConfig.from_hydra_config(cfg)
    dagspace_key = hooks.wandb_dagspace(cfg)
    pipeline_id = (
        wl.pipeline_run_id(cfg, dagspace=dagspace_key) if dagspace_key else None
    )
...
def await_slurm_result(job: Any, cfg: DictConfig, node_key: str) -> StageResult:
    ...
    try:
        ...
        if missing_result:
            wait_s = int(
                OmegaConf.select(cfg, "runtime.submitit_result_wait_s", default=300)
            )
            print_status(
                {
            ...
            if os.path.exists(result_path):
                ...
                job_result = _result
                print_status(
                    {"debug": "recovered_result_after_wait", "job_id": job.job_id}
                )
            ...
            try:
                ...
                if state in ("R", "PD", "CG"):
                    ...
                    while True:
                        ...
                        if not check.stdout.strip() or check.stdout.strip() not in (
                            "R",
                            "PD",
                            "CG",
                        ):
                ...
                if os.path.exists(result_path):
                    ...
                    print_status(
                        {
                            "debug": "recovered_result_after_squeue_wait",
                            "job_id": job.job_id,
                        }
                    )
...
def execute_stage_job(context_data: dict[str, Any]) -> dict[str, Any]:
    ...
    run_config = build_run_config(
        ...
        dagspace_name=hooks.dagspace_name,
    )

    with make_wandb_logger(
        cfg, hooks, stage=node.stage, run_id=wandb_run_id, run_config=run_config
    ) as logger:
        ...
        if result.outputs and "dataset" in result.outputs:
            try:
                ...
                safe_log_table(logger, df_out, f"{node.stage}/results")
            except Exception as e:
                print(
                    f"Warning: Failed to log output table for {node.key}: {e}",
                    flush=True,
                )
...
def run_experiment(cfg: DictConfig, hooks: OrchestratorHooks) -> None:
    ...
    with make_wandb_logger(
        cfg, hooks, stage="orchestrator", run_id="monitor"
    ) as logger:
        try:
            ...
            for node_key in ordered_nodes:
                ...
                if node.launcher and _SUBMITIT_AVAILABLE:
                    ...
                    launcher_cfg = load_launcher_config(
                        cfg, node.launcher, config_dir=hooks.config_dir
                    )
                    ...
                    executor = create_submitit_executor(
                        launcher_cfg,
                        f"{hooks.job_prefix}-{node.key}",
                        log_folder,
                        use_srun=hooks.use_srun,
                    )
