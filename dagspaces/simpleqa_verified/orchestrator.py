"""Orchestrator for the SimpleQA-Verified dagspace.

Copied from goldcoin_hipaa/orchestrator.py with dagspace-specific renames
and a metrics flattener tailored to the SimpleQA scalar metric shape
(correct/incorrect/not_attempted/attempted_rate/accuracy_given_attempted/f1).
"""
...
def _get_wandb_logger(cfg: DictConfig, stage: str, run_id: Optional[str] = None, run_config: Optional[Dict[str, Any]] = None):
    ...
    pipeline_id = pipeline_run_id(cfg, dagspace="simpleqa_verified")
...
def execute_stage_job(context_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    run_config = build_run_config(cfg, node, context.inputs, context.output_paths, dagspace_name="simpleqa_verified")
...
def run_experiment(cfg: DictConfig) -> None:
    """Execute the SimpleQA-Verified evaluation pipeline."""
    with _get_wandb_logger(cfg, stage="orchestrator", run_id="monitor") as logger:
        try:
            ...
            if hydra_output_dir:
                output_root = os.path.join(hydra_output_dir, "simpleqa_verified")
            else:
            ...
            for node_key in ordered_nodes:
                ...
                if node.launcher and _SUBMITIT_AVAILABLE:
                    ...
                    executor = _create_submitit_executor(launcher_cfg, f"SimpleQAVerified-{node.key}", log_folder, use_srun=False)
