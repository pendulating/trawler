...
# Note: norm_extraction excluded because reasoning output has complex nested types
# that cause PyArrow schema conflicts when Ray reads directly
_STREAMING_COMPATIBLE_STAGES = {"classify_relevance", "taxonomy", "verification", "norm_reasoning"}
...
def _ensure_ray_init_with_cpu_limits(cfg: DictConfig) -> None:
    ...
    # Fix uvloop conflict with Ray in Python 3.12+
    # uvloop's get_event_loop() raises RuntimeError when no loop exists
    # We need to reset to asyncio's default policy before Ray starts
    try:
        import asyncio
        # Check if uvloop is installed and active
        policy = asyncio.get_event_loop_policy()
        if 'uvloop' in type(policy).__module__:
            asyncio.set_event_loop_policy(asyncio.DefaultEventLoopPolicy())
            print("[Ray init] Reset event loop policy from uvloop to asyncio default", flush=True)
    except Exception as e:
        print(f"[Ray init] Warning: Could not reset event loop policy: {e}", flush=True)
    
    # Set critical Ray and vLLM environment variables for Slurm/multi-GPU stability
    os.environ["RAY_TMPDIR"] = "/tmp"
    os.environ["VLLM_NO_SHM_BROADCAST"] = "1"  # Avoid hangs in shared memory communication
    ...
    try:
        # Priority 1: Actual SLURM allocation
        slurm_mem = os.environ.get("SLURM_MEM_PER_NODE") or os.environ.get("SLURM_MEM_PER_CPU")
        if slurm_mem:
            job_mem_gb = int(int(slurm_mem) / 1024)
        else:
            # Priority 2: Config explicitly
            job_mem_gb = int(getattr(cfg.runtime, "job_memory_gb", 64) or 64)
    except Exception:
        job_mem_gb = 64
    
    try:
        # Be more conservative with object store if memory is tight (< 32GB)
        ratio = 0.90 if job_mem_gb >= 32 else 0.50
        obj_store_bytes = int(max(1, job_mem_gb) * (1024 ** 3) * ratio)
    except Exception:
        obj_store_bytes = int(8 * (1024 ** 3) * 0.50)
    
    namespace = os.environ.get("RAY_NAMESPACE") or os.environ.get("WANDB_GROUP") or "historical_norms"
    ...
    try:
        ...
        # Fix uvloop conflict: Ray workers need asyncio's default event loop policy
        # Set env var that tells Ray to use default asyncio event loop
        init_kwargs["runtime_env"] = {
            "env_vars": {
                # Disable uvloop in Ray workers
                "RAY_USE_UVLOOP": "0",
            },
        }
        
        # In SLURM monitor, we might have very limited resources.
        # Ensure we don't request more than available if we can detect it.
        try:
            if os.environ.get("SLURM_JOB_NAME") == "HNORMS-monitor":
                # Force very conservative limits for monitor
                init_kwargs["num_cpus"] = min(init_kwargs.get("num_cpus", 2), 2)
                init_kwargs["object_store_memory"] = min(init_kwargs["object_store_memory"], int(4 * (1024**3)))
        except Exception:
            pass
...
def _prepare_streaming_dataset(dataset_path: str, columns: Mapping[str, str], cfg: DictConfig, stage: str) -> tuple[Optional[Any], bool]:
    ...
    try:
        ...
        if not ray.is_initialized():
            ...
            namespace = os.environ.get("RAY_NAMESPACE") or os.environ.get("WANDB_GROUP") or "historical_norms"
            try:
                # Detect SLURM resources for fallback init
                cpus_alloc = None
                gpus_alloc = None
                try:
                    cpt = os.environ.get("SLURM_CPUS_PER_TASK")
                    if cpt: cpus_alloc = int(cpt)
                    cv = os.environ.get("CUDA_VISIBLE_DEVICES")
                    if cv: gpus_alloc = len([x for x in cv.split(",") if x.strip()])
                except Exception: pass
                
                init_kwargs = {"log_to_driver": True, "namespace": str(namespace)}
                if cpus_alloc: init_kwargs["num_cpus"] = cpus_alloc
                if gpus_alloc is not None: init_kwargs["num_gpus"] = gpus_alloc
