#!/bin/bash
# Gold-label regeneration (fiction10 flows) with google/gemma-4-31B-it
...
# Estimate: ~1h reasoning + ~1h extraction (2,993 chunks).
#
...
#
# W&B: project historical-norms-extraction, run fiction10_flows_gemma4_<stage suffixes>.
...
echo "[fiction10_flows_gemma4] chunks : $FICTION_CHUNKS_PATH"
echo "[fiction10_flows_gemma4] model  : gemma-4-31b/instruct | pipeline: COLM_flows_fiction_prefetched_gemma4"

python -m dagspaces.historical_norms.cli \
  pipeline=COLM_flows_fiction_prefetched_gemma4 \
  model=gemma-4-31b/instruct \
  experiment.name=fiction10_flows_gemma4
