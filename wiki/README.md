...
**Method deep-dives**
- [2026-07-31_kto_plan.md](2026-07-31_kto_plan.md) — **⚠️ READ §19 FIRST: the camera-ready model ruling (2026-08-05)** — GRPO = m2 `full`, KTO = k3 `verdict`, v9-ckpt100 deprecated, both promoted over failed gates because the benchmarks are too noisy to arbitrate. §§1–18 are the k-series pre-registration and results: offline prospect-theoretic tuning on norm-grounded binary desirability labels, label-corrected counterfactual pairs from the audited m2 reward stack, held-out chunk split, and a mandatory corrected-SFT control; plus §17's gold-validity audit, which reframes the construct mismatch as a positive result about normative grounding
- [grpo_redesign/](grpo_redesign/README.md) — modular GRPO redesign (m-series). **SHIPPING as of 2026-08-05**: the m2 `full` cell is the camera-ready GRPO model, promoted by author ruling over its own failed gates ([2026-07-31_kto_plan.md §19](2026-07-31_kto_plan.md)). This tree is the design spec for what the paper reports, and also the ablation/limitations record: the m1 wave was invalidated by four stack defects, and the repaired m2 wave-A ran clean while still holding every cell near the blanket floor (post-mortems: `notebooks/normative-simulacra/grpo_m{1,2}_wave_analysis_*.py`)
- [2026-08-05_distilled_grounding_plan.md](2026-08-05_distilled_grounding_plan.md) — **PLAN, not yet run.** Re-run the two-stage `historical_norms` flow pipeline with the fine-tuned checkpoints (base / SFT / m2-full / k3-verdict) substituted for the Gemma-4-31B teacher, then recompute the §5.2 grounding reclassification rate per arm. Tests whether normative grounding was distilled into the weights. Records the measured training contamination: GRPO saw only 492 chunks across 4 of 10 novels, KTO holds out 599, SFT saw all of fiction-10 — so **top100 is the primary chunk set and fiction-10 is descriptive only**
- [grpo-reward.md](grpo-reward.md) — **HISTORICAL as of 2026-08-05** (the v9–v12a lineage is deprecated; the camera-ready reward is `grpo_redesign/`). Composite reward, directional composition, listwise R_ground, deontic tier ladder — **rewritten 2026-07-20 to be 1:1 with the code**, incl. a deviations-from-vanilla-GRPO checklist for the camera-ready)
