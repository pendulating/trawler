...
> **Cost-sensitive tiers on prohibited-governed flows (v10 2026-06-24, v12a 2026-07-03):** the v9 multiplier is symmetric, but the governing norms are ~4:1 appropriate:inappropriate, so the EV-optimal when-unsure verdict is permissive. v10 adds `rground_app_floor_prohibit: 0.1` — a **false-permit** (prohibited-governed flow called "appropriate") floors at 0.1 instead of 0.4. v10/v11 forensics then showed the floor priced the wrong-commit *tail* but left hedging the safe optimum (prohibited-flow hedge mass frozen ~72% across both runs; GoldCoin Forbid recall pinned at 0.55 < SFT 0.65 for three iterations), so v12a adds `rground_app_hedge_prohibit: 0.5`: a **hedge** ("ambiguous"/missing/unrecognized verdict) on a prohibited/discouraged-governed flow drops from the neutral 0.7 tier to 0.5, widening the commit-vs-hedge gap 0.3→0.5 exactly where it binds. Hedges elsewhere keep 0.7; **no-flow declarations stay neutral** (abstention economics unchanged). Tier ladder on a prohibited-governed flow: correct 1.0 / hedge 0.5 / false-permit 0.1. Both knobs live in `deontic.appropriateness_multiplier`; `null` disables each (hedge-null ≡ v10, both-null ≡ v9). Changing either bumps the prescreen signature; the formula version is `v12a_cost_sensitive_hedge`. See `wiki/grpo_training_field_notes/2026-06-24_v10_plan.md`, `2026-07-03_v12a_plan.md`.
...
> **Vignette rows are scored separately.** Rows mixed in via `vignette_ratio` (`task_type: norm_judgment`) bypass the 6-component composite entirely (neither additive nor gated) and receive a direct 3-component judgment reward: `0.5·r_judgment + 0.25·r_judgment_reasoning + 0.25·r_norm_cite` (`judgment_reward_weights`). The R = Σ wᵢRᵢ formula applies only to CI-extraction rows — say so in the paper. Also note: pre-screening can strip vignettes disproportionately — and force-asymmetrically (v10: pool 3.07:1 yes:no → realized 5.2:1) — so the configured `vignette_ratio: 0.3` is the *pre-screen* mix; the realized training-set mix is recorded in `training_metadata.json` as `n_vignettes_post_screen` (vs `n_vignettes_pre_screen`) and the realized **force** mix as `n_vignettes_{yes,no}_{pre,post}_screen` (also in `prescreen_report.json` as `vignette_{yes,no}_{in,kept}`). Per-call per-gold-class verdict accuracy/drift streams to W&B under `vignette/*`; offline forensics over reward traces: `scripts/analyze_grpo_verdict_traces.py` (see `wiki/grpo_training_field_notes/2026-07-01_v11_probe_midrun_forensics.md`).
